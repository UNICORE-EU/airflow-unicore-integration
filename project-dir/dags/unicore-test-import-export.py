...
with DAG(
    ...
    t1 = uc_ops.UnicoreExecutableOperator(
        name="task 1", task_id="1", executable="cat date.json ", imports=imports
    )
    t2 = uc_ops.UnicoreExecutableOperator(
        name="task 2", task_id="2", executable="curl -X GET http://date.jsontest.com/"
    )
    t3 = uc_ops.UnicoreExecutableOperator(
        name="task 3", task_id="3", executable="date >> date.txt", exports=exports
    )
